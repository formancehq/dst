...
func TestTransactionWindow(t *testing.T) {
	...
	if got := transactionWindow(s, nil, 2, false, 10); len(got) != 2 || got[0] != 2 || got[1] != 1 {
		t.Fatalf("DESC frontier=2 = %v, want [2 1]", got)
	}
	if got := transactionWindow(s, nil, 3, true, 10); len(got) != 3 || got[0] != 1 || got[2] != 3 {
		t.Fatalf("ASC = %v, want [1 2 3]", got)
	}
	if got := transactionWindow(s, nil, 3, true, 2); len(got) != 2 || got[1] != 2 {
